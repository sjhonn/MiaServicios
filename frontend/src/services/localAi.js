// Ejecuta las operaciones NLP directamente en el navegador.
...
export const executeLocalAi = (type, payload) => {
  ...
  const operations = {
    summarize: () => {
      const summary = summarize(payload.text, payload.sentences);
      return {
        summary,
        originalCharacters: payload.text.length,
        summaryCharacters: summary.length,
        reductionPercent: Number(Math.max(0, (1 - summary.length / payload.text.length) * 100).toFixed(1))
      };
    },
    sentiment: () => sentiment(payload.text),
    keywords: () => keywords(payload.text, payload.limit),
    classify: () => classify(payload.text),
    statistics: () => statistics(payload.text),
    normalize: () => normalize(payload.text, payload.casing)
  };

  if (!operations[type]) throw new Error('Operacion no disponible.');

  return {
    result: operations[type](),
    processingMs: Number((performance.now() - startedAt).toFixed(2)),
    engine: 'mia-nlp-browser'
  };
};
